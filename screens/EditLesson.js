...
const EditLesson = () => {
    ...
    useEffect(() => {
        if (!lesNum) return;
        axios.get(`http://192.168.45.169:5000/api/lesson-api/${lesNum}`)
            .then(res => {
                const data = res.data;
                ...
                setOriginThumb(`http://192.168.45.169:5000/img/${data.lesThumbImg}`);
                setOriginBg(`http://192.168.45.169:5000/img/${data.lesBackgroundImg}`);
            })
            .catch(err => {
    ...
    const handleUpdate = async () => {
        ...
        try {
            await axios.put(`http://192.168.45.169:5000/api/lesson-api/${lesNum}`, formData, {
