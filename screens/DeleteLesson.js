...
const DeleteLesson = () => {
    ...
    const fetchLessons = async () => {
        try {
            const res = await axios.get(`http://192.168.45.169:5000/api/lesson/instructor/${instNum}`);
            setLessons(res.data);
        } catch (err) {
                        ...
                        Alert.alert(
                            ...
                            [
                                { text: '아니오', style: 'cancel' },
                                {
                                    ...
                                    onPress: async () => {
                                        try {
                                            await axios.delete(`http://192.168.45.169:5000/api/lesson/${lesNum}`);
                                            Toast.show('레슨 삭제 완료', {
                                                duration: 3000,
    ...
    const renderItem = ({ item }) => (
        <TouchableOpacity onPress={() => handleDelete(item.lesNum)}>
            <LessonCard>
                <LessonImage source={{ uri: `http://192.168.45.169:5000/img/${item.lesThumbImg}` }} />
                <LessonInfoContainer>
