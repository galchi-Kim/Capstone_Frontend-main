...
const Certification = () => {
    ...
    useEffect(() => {
        axios.get('http://10.32.10.195:3000/api/lessons', {
            params: {
                lesPlace: place,
                lesLevel: level
            }
    ...
    return (
        <BeginnerScreenContainer>
            <View style={{ alignSelf: 'flex-start', paddingLeft: 5 }}>
                <PageTitle style={{ color: 'black' }}>{level}</PageTitle>
            </View>
                ...
                renderItem={({ item }) => (
                    <TouchableOpacity onPress={() => navigation.navigate('LessonDetail', { lesson: item })}>
                        <View style={{ flexDirection: 'row', padding: 15, borderBottomWidth: 1, alignItems: 'center' }}>
                            <Image source={{ uri: `http://10.32.10.195:3000/img/${item.lesThumbImg}` }} style={{ width: 80, height: 70, marginRight: 20, borderRadius: 10 }}/>
                            <View style={{ flex: 1 }}>
