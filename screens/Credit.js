...
const Credit = () => {
    ...
    const handlePayment = async () => {
        try {
            ...
            // 주문 등록
            await axios.post('http://192.168.45.169:5000/api/order', {
                userId,
                lessonId: lesson.lesNum
            });

            // 장바구니 삭제 요청
            await axios.delete(`http://192.168.45.169:5000/api/cart/user/${userId}/lesson/${lesson.lesNum}`);
    ...
    return (
        <View style={{ flex: 1, backgroundColor: '#fbfff4' }}>
            <ScrollView contentContainerStyle={{ paddingBottom: 100 }}>
                ...
                <LessonInfoContainer>
                    <LessonThumbnail source={{ uri: `http://192.168.45.169:5000/img/${lesson.lesThumbImg}` }} />
                    <LessonDetailTextContainer>
                ...
                <View style={{ alignItems: 'center' }}>
                    <LessonProfileImage
                        source={{ uri: `http://192.168.45.169:5000/img/${lesson.userImg}` }}
