...
const InstDetailProfile = () => {
    ...
    useEffect(() => {
        ...
        axios
            .get(`http://192.168.45.169:5000/api/users/instructor/${encodeURIComponent(instName)}`)
            .then((res) => setInstructor(res.data))
            .catch((err) => console.error('강사 정보 불러오기 실패:', err));

        axios
            .get(`http://192.168.45.169:5000/api/users/instructor/${encodeURIComponent(instName)}/lessons`)
    ...
    return (
        <styles.LessonDetailContainer>
            ...
            <ScrollView contentContainerStyle={{ flexGrow: 1, alignItems: 'center' }}>
                ...
                {/* 강사 프로필 이미지 */}
                <styles.LessonProfileImage source={{ uri: `http://192.168.45.169:5000/img/${instructor.userImg}` }} />
